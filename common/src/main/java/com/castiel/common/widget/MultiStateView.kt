...
class MultiStateView @JvmOverloads constructor(
    ...
    init {
        ...
        val loginViewResId = a.getResourceId(R.styleable.MultiStateView_msv_loginView, -1)
        if (loginViewResId > -1) {
            val inflatedLoadingView = inflater.inflate(loginViewResId, this, false)
            loginView = inflatedLoadingView
        ...
        viewState = when (a.getInt(R.styleable.MultiStateView_msv_viewState, VIEW_STATE_CONTENT)) {
            ...
            VIEW_STATE_LOGIN -> ViewState.LOGIN
    ...
    fun setViewForState(view: View, state: ViewState, switchToState: Boolean = false) {
        when (state) {
            ViewState.LOGIN -> {
                if (loginView != null) removeView(loginView)
                loginView = view
                addView(view)
            }
    ...
    private fun isValidContentView(view: View): Boolean {
        return if (contentView != null && contentView !== view) {
            false
        } else view != loginView && view != errorView && view != emptyView
    }
    ...
    private fun setView(previousState: ViewState) {
        when (viewState) {
            ViewState.LOGIN -> {
                requireNotNull(loginView).apply {
...
private const val VIEW_STATE_LOGIN = 3
